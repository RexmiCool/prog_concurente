...
void *server_thread(void *arg) {
  ...
  while (!stop) {
    ...
    if (strstr(path, "1")) {
      ...
      // Modify the message by adding process ID
      strcat(recv_buffer1.buffer, process_id);
      recv_buffer1.size = strlen(recv_buffer1.buffer) + 1;

      // Signal the client thread that the message is ready
      pthread_cond_signal(&cond1);
      pthread_mutex_unlock(&mutex2);

      recv_buffer1.size = 0; // Reset the buffer size
    } else {
      pthread_mutex_lock(&mutex2);
      ...
      recv_buffer2.size = 0; // Reset the buffer size
    }
...
void *brain_thread(void *arg) {
  ...
  while (!stop) {
    if (strstr(path, "1")) {
      pthread_mutex_lock(&mutex1);
      while (recv_buffer1.size == 0 && !stop) {
        // Wait for a message from the client thread
        pthread_cond_wait(&cond1, &mutex1);
      }
      ...
      // Copy the modified message to the send buffer
