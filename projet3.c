...
Buffer brain_buffer1 = { .size = 0 };
Buffer brain_buffer2 = { .size = 0 };

pthread_mutex_t mutex1 = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex2 = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t brain_mutex1 = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t brain_mutex2 = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t cond1 = PTHREAD_COND_INITIALIZER;
pthread_cond_t cond2 = PTHREAD_COND_INITIALIZER;
pthread_cond_t brain_cond1 = PTHREAD_COND_INITIALIZER;
pthread_cond_t brain_cond2 = PTHREAD_COND_INITIALIZER;
...
void *brain_thread(void *arg) {
    ...
    while (!stop) {
        if (strstr(path, "1")) {
            pthread_mutex_lock(&brain_mutex1);
            while (brain_buffer1.size == 0 && !stop) {
                pthread_cond_wait(&brain_cond1,
                &brain_mutex1);
            }

            if (stop) {
                pthread_mutex_unlock(&brain_mutex1);
                break;
            }

            printf("[%s - Brain] Processing message: %s\n", process_id, brain_buffer1.buffer);
            // Here you can modify the message in brain_buffer1.buffer if needed

            pthread_mutex_lock(&mutex1);
            memcpy(send_buffer1.buffer, brain_buffer1.buffer, brain_buffer1.size);
            send_buffer1.size = brain_buffer1.size;
            pthread_cond_signal(&cond1);
            pthread_mutex_unlock(&mutex1);

            brain_buffer1.size = 0;
            pthread_mutex_unlock(&brain_mutex1);
        } else {
            pthread_mutex_lock(&brain_mutex2);
            while (brain_buffer2.size == 0 && !stop) {
                pthread_cond_wait(&brain_cond2, &brain_mutex2);
            }

            if (stop) {
                pthread_mutex_unlock(&brain_mutex2);
                break;
            }

            printf("[%s - Brain] Processing message: %s\n", process_id, brain_buffer2.buffer);
            // Here you can modify the message in brain_buffer2.buffer if needed

            pthread_mutex_lock(&mutex2);
            memcpy(send_buffer2.buffer, brain_buffer2.buffer, brain_buffer2.size);
            send_buffer2.size = brain_buffer2.size;
            pthread_cond_signal(&cond2);
            pthread_mutex_unlock(&mutex2);

            brain_buffer2.size = 0;
            pthread_mutex_unlock(&brain_mutex2);
        }
    }
    ...
    while (!stop) {
        ...
        if (strstr(path, "1")) {
            ...
            if (recv_buffer2.size == -1) {
                ...
                pthread_mutex_lock(&brain_mutex2);
                memcpy(brain_buffer2.buffer, recv_buffer2.buffer, recv_buffer2.size);
                brain_buffer2.size = recv_buffer2.size;
                pthread_cond_signal(&brain_cond2);
                pthread_mutex_unlock(&brain_mutex2);
            }
            pthread_mutex_unlock(&mutex2);
        } else {
            ...
            if (recv_buffer1.size == -1) {
                ...
                pthread_mutex_lock(&brain_mutex1);
                memcpy(brain_buffer1.buffer, recv_buffer1.buffer, recv_buffer1.size);
                brain_buffer1.size = recv_buffer1.size;
                pthread_cond_signal(&brain_cond1);
                pthread_mutex_unlock(&brain_mutex1);
            }
            pthread_mutex_unlock(&mutex1);
        }
...
int main() {
    ...
    pthread_mutex_destroy(&brain_mutex1);
    pthread_mutex_destroy(&brain_mutex2);
    pthread_cond_destroy(&cond1);
    pthread_cond_destroy(&cond2);
    pthread_cond_destroy(&brain_cond1);
    pthread_cond_destroy(&brain_cond2);
