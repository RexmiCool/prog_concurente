...
int func_client_thread(pthread_mutex_t mutex, Buffer send_buffer, int stop, pthread_cond_t cond, char* process_id, int sockfd)
{
...
void *client_thread(void *arg)
{
    ...
    while (!stop)
    {
        ...
        if (strstr(path, "1"))
        {
            if (func_client_thread(mutex1, send_buffer1, stop, cond1, process_id, sockfd) == 1)
            {
                break;
            }
        }
        ...
        {
            if (func_client_thread(mutex2, send_buffer2, stop, cond2, process_id, sockfd) == 1)
            {
                break;
            }
        }
...
void *brain_thread(void *arg)
{
    ...
    while (!stop)
    {
        if (strstr(path, "1"))
        {
            ...
            recv_buffer2.size = 0;
            pthread_mutex_unlock(&mutex2);
            printf("[%s - Brain] Traited message: %s\n", process_id, send_buffer1.buffer);
        }
        else
        {
            pthread_mutex_lock(&mutex1);
